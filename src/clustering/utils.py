...
class Partition(Generic[T_co]):
    ...
    def __init__(
        self, G: Graph, sets: list[set[T_co]], node_part: dict[T_co, int], degree_sums: list[int], weight: Union[None, str] = DataKeys.WEIGHT
    ) -> None:
        """
        ...
        self._weight: Union[None, str] = weight
        self._partition_degree_sums: list[int] = degree_sums

    @classmethod
    def from_partition(cls, G: Graph, 𝓟: Union[Collection[Collection[T_co]], Partition[T_co]], weight: Union[None, str] = None) -> Partition[T_co]:
        """Create a new partition of the graph G, given by the nodes in the partition 𝓟 of G's nodes."""
    ...
    @classmethod
    def singleton_partition(cls, G: Graph, weight: Union[None, str] = None) -> Partition[T_co]:
    ...
    @staticmethod
    def is_partition(G: Graph, 𝓟: Union[Collection[Collection[T_co]], Partition[T_co]]) -> bool:
    ...
    def adjacent_communities(self, v: T_co) -> set[frozenset[T_co]]:  # type: ignore
        ...
        neighbor_community_ids = {self._node_part[u] for u in self.G[v]}.union({self._node_part[v]})
...
def single_node_neighbor_cut_size(G: Graph, v: S, D: Set[S], weight: Union[str, None] = None) -> float:
    """
...
def preprocess_graph(G: Graph, weight: Union[str, None]) -> Graph:
